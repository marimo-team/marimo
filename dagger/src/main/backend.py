...
class Backend:
    ...
    async def test(self) -> dagger.Container:
        env = (
            ...
            .with_exec(["cp", "frontend/index.html", "marimo/_static/index.html"])
            .with_exec(["cp", "frontend/public/favicon.ico", "marimo/_static/favicon.ico"])
            .with_exec(["make", "py"])
        ...
        # test:test
        await env.with_exec(["hatch", "run", "+py=3.12", "test:test", "-v", "tests/", "-k", "not test_cli"]).sync()

        # test-optional:test
        return env.with_exec(["hatch", "run", "+py=3.12", "test-optional:test", "-v", "tests/", "-k", "not test_cli"])
