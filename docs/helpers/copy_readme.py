...
class ReadmePartDirective(Directive):
    ...
    def run(self):
        ...
        next_match = re.search(next_header_with_same_level, content[match.end():], re.IGNORECASE)

        stripped_content = ''
        if match:
            # Extract everything below the header, up to the next header
            if next_match and next_match.group(0):
                stripped_content = content[match.end():match.end() + next_match.start()].strip()
            else:
                stripped_content = content[match.end():].strip()

        # Update asset links, add a leading slash if needed
        stripped_content = stripped_content.replace("/docs/_static/", "/_static/").replace("docs/_static/", "/_static/")
