...
def __():
    ...
    return mo, load, generate, Path, snapshot_download
    ...
    def get_model_path(path_or_hf_repo: str) -> Path:
        ...
        if model_path.exists():
            ...
            try:
                ...
                raise ValueError(f"Error downloading model from Hugging Face: {str(e)}")
...
def __(MODEL_ID, get_model_path, load, load_model_button, mo):
    ...
    try:
        mo.md("⏳ Fetching model... This may take a while if downloading from Hugging Face.")
        model_path = get_model_path(MODEL_ID.value)
        mo.md(f"📁 Model path: {model_path}")
        mo.md("🔄 Loading model into memory...")
        model, tokenizer = load(model_path)
        status = mo.md(f"✅ Model loaded successfully!")
    except Exception as e:
        status = mo.md(f"❌ Error loading model: {str(e)}")
        raise
...
def __(mo):
    ...
    return (MODEL_ID, system_message,)


@app.cell
def __(mo):
    temp_slider = mo.ui.slider(start=0.0, stop=2.0, step=0.1, value=0.7, label="Temperature Slider")
    max_tokens = mo.ui.number(value=512, label="Max Tokens Per Turn")
    
    temp_slider, max_tokens # display the inputs
    return(temp_slider, max_tokens,)



@app.cell
def __(generate, mo, model, tokenizer, system_message, temp_slider, max_tokens):
    def mlx_chat_model(messages, config):
            # Include the system message as the first message
            chat_messages = [
                {"role": "system", "content": system_message.value}
            ]
