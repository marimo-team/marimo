...
export const MockRequestClient = {
  create(overrides?: Partial<RequestClient>): RequestClient {
    return {
      ...
      getBuildPreview: vi.fn().mockResolvedValue({ success: true, cells: [] }),
      runBuild: vi.fn().mockResolvedValue({ success: true, buildId: "" }),
      cancelBuild: vi.fn().mockResolvedValue({ success: true }),
