...
final = outer_function()`;
    ...
    it("should unfold multiple separate folded regions when errors are in different regions", () => {
      const cellId = "cell1" as CellId;
...
result = function1() + function2()`;
      ...
      // Fold function1 (lines 2-3)
      // @ts-expect-error - foldCode accepts position as second arg at runtime
      const fold1Result = foldCode(view, view.state.doc.line(1).from);
      expect(fold1Result).toBe(true);

      // Fold function2 (lines 6-7)
      // @ts-expect-error - foldCode accepts position as second arg at runtime
      const fold2Result = foldCode(view, view.state.doc.line(5).from);
      expect(fold2Result).toBe(true);

      // Verify both are folded
      const foldedBefore = foldedRanges(view.state);
      let foldedCount = 0;
      foldedBefore.between(0, view.state.doc.length, () => {
        foldedCount++;
      });
      expect(foldedCount).toBeGreaterThanOrEqual(2);

      // Add errors in both folded regions
      const mockObservable = errorsObservable as Observable<
        TracebackInfo[] | undefined
      > & { set: (value: TracebackInfo[] | undefined) => void };
      mockObservable.set([
        {
          kind: "cell",
          cellId,
          lineNumber: 2, // Inside function1
        },
        {
          kind: "cell",
          cellId,
          lineNumber: 6, // Inside function2
        },
      ]);
      ...
      // Verify both regions are unfolded
      const foldedAfter = foldedRanges(view.state);
      let stillFolded = false;
      foldedAfter.between(0, view.state.doc.length, () => {
        stillFolded = true;
