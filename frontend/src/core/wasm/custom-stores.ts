...
function createFileStore(className: string): FileStore | null {
  ...
  if (!FileStoreClass) {
    ...
    Logger.warn(
      "Define file store classes globally: window.MyFileStore = class MyFileStore { ... }",
    );
    return null;
  }
  ...
  if (typeof FileStoreClass !== "function") {
    Logger.error(
      `File store class "${className}" is not a constructor function`,
    );
    return null;
  }
...
export function initializeCustomFileStores() {
  ...
  if (customStores.length > 0) {
    Logger.log(
      `🗄️ Injected ${customStores.length} custom file store(s) into notebookFileStore`,
    );
  }
}
