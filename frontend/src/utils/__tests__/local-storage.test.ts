...
describe("NotebookScopedLocalStorage", () => {
  ...
  it("should initialize with the default value", () => {
    storage = new NotebookScopedLocalStorage("test", schema, getDefaultValue);
    expect(storage?.get()).toEqual(getDefaultValue());
  });

  it("should return default value after remove is called", () => {
    storage = new NotebookScopedLocalStorage("test", schema, getDefaultValue);
    // Set a custom value first
    storage.set({ name: "Jane Doe", age: 25 });
    expect(storage.get()).toEqual({ name: "Jane Doe", age: 25 });
  ...
  it("should scope storage to filename when filename is set", () => {
    // Set initial filename
    store.set(filenameAtom, "notebook.py");

    // Create storage with filename
    storage = new NotebookScopedLocalStorage("test", schema, getDefaultValue);
    expect(storage.getKey()).toEqual("test:notebook.py");
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        expect(storage!.getKey()).toEqual("test:other.py");
        expect(storage!.get()).toEqual({ name: "Jane Doe", age: 25 });
        resolve(undefined);
      }, 0);
    });
