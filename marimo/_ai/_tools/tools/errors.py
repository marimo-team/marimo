...
class GetNotebookErrors(
    ...
    def handle(self, args: GetNotebookErrorsArgs) -> GetNotebookErrorsOutput:
        ...
        summaries = context.get_notebook_errors(
            session_id, include_stderr=True
        )

        total_errors = self._get_total_errors_count_without_stderr(summaries)
        total_cells_with_errors = len(summaries)
    ...
    def _get_total_errors_count_without_stderr(
        self, summaries: list[MarimoCellErrors]
    ) -> int:
        return sum(
            len([e for e in s.errors if e.type != "STDERR"]) for s in summaries
        )
