...
NOTICE = "# This section is auto-generated by marimo."
...
class RemoveMarimoImport(ast.NodeTransformer):
    def visit_Import(self, node: ast.Import) -> Optional[ast.Import]:
        node.names = [
            alias
            for alias in node.names
            if (alias.asname and alias.asname != "marimo")
            or (not alias.asname and alias.name != "marimo")
        ]
...
def build_import_section(import_blocks: list[str]) -> str:
    ...
    tree = ast.parse("\n".join(import_blocks))
    tree = RemoveMarimoImport().visit(tree)
    stripped_block = ast.unparse(tree)
