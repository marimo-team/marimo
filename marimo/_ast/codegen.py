...
# NOTE: Changing this notice will break the ability to detect the location of
# the generated imports. Be careful.
NOTICE_GUARD = "# 👋 This file was generated by marimo."
# TODO: Better notice. A little rambley and not clear as it could be.
NOTICE = "\n".join(
    [
        NOTICE_GUARD + " You can edit it, and tweak",
...
def to_functiondef(
    ...
    definition_body = to_general_functiondef(cell, name, allowed_refs, fn)
...
def generate_filecontents(
    ...
    if header_comments is not None:
        if not toplevel_fn:
            filecontents = [header_comments, ""]
        elif header_comments := header_comments.rstrip():
            filecontents = [header_comments, "", NOTICE, ""]
        else:
            # If the header comments are empty, we may still need to contain the
            # notice to separate the imports from the rest of the code.
            filecontents = [NOTICE, ""]

    filecontents.append("\n\n".join(import_blocks))
    if "marimo" not in toplevel_imports:
        filecontents.extend(["", "import marimo"])
    filecontents.extend(
        [
            "\n",
            f'__generated_with = "{__version__}"',
...
def get_header_comments(filename: str) -> Optional[str]:
    ...
    invalid, which is determined by:
        1. If the file is does not contain the marimo
            import statement
        2. If the section before the marimo notice
    ...
    if "import marimo" not in contents:
        return None
    elif NOTICE_GUARD in contents:
        if contents.startswith(NOTICE_GUARD):
            return ""
        header, _ = contents.split("\n" + NOTICE_GUARD, 1)
    else:
