...
def validate_name(name: str, allow_new_file: bool) -> str:
    """
    ...
    if _is_github_issue_url(name):
        return _handle_github_issue(name)
    ...
    if _is_github_py(name):
        return _handle_github_py(name)

    if is_url(name):
        return _create_tmp_file_from_url(name)
...
def _handle_github_issue(url: str) -> str:
...
def _handle_github_py(url: str) -> str:
    # Change hostname to raw.githubusercontent.com
    path = urllib.parse.urlparse(url).path
    path = path.replace("/blob/", "/", 1)
    url = f"https://raw.githubusercontent.com{path}"
    path_to_app = _create_tmp_file_from_url(url)
    return path_to_app


def _create_tmp_file_from_url(url: str) -> str:
    d = tempfile.TemporaryDirectory()
    logging.info("Downloading %s", url)
    path_to_app = os.path.join(d.name, os.path.basename(url))
    urllib.request.urlretrieve(url=url, filename=path_to_app)
    logging.info("App saved to %s", path_to_app)
    return path_to_app


def _create_tmp_file_from_content(content: str, name: str) -> str:
    d = tempfile.TemporaryDirectory()
    logging.info("Creating temporary file")
    path_to_app = os.path.join(d.name, name)
