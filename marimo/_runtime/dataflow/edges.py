...
class EdgeComputer:
    """Computes graph edges based on cell definitions and references.

    Responsibilities:
    - Compute parent/child relationships from defs/refs
    - Handle SQL reference resolution
    - Handle deleted reference semantics
    - Apply language-specific edge rules (Python/SQL)
    """
    ...
    def compute_edges_for_cell(
        ...
        for name in cell.refs:
            ...
            other_ids_defining_name: set[CellId_t] = (
                self.definitions.definitions.get(name, set()) - {cell_id}
            )
            ...
            if sql_ref:
                sql_matches = self.definitions.find_sql_hierarchical_matches(
                    sql_ref
                )
