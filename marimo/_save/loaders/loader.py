...
class Loader(ABC):
    ...
    def storage_dir(self) -> Path | None:
        """Cache directory this loader writes its block into, if any.

        `None` when the cache keeps nothing on the local filesystem, so there
...
class BasePersistenceLoader(Loader):
    ...
    def clearable_bytes(self) -> int:
        ...
        if root is None:
            return 0
        return sum(entry_bytes(path) for path in self._clearable_paths(root))
