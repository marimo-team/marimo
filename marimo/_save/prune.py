...
def apply_prune(plan: PrunePlan) -> CacheDirStats:
    ...
    The plan was made before anyone agreed to it, and a kernel can record
    entries in between. Each directory is planned again on the spot, and
    only what both plans doom is deleted, so an entry recorded under live
    code meanwhile is kept and its record survives. The agreement given
    to the first plan covers the directory, not a list of files.
    ...
    for directory in plan.directories:
        fresh = _plan_directory(directory.cache_dir)
        freed += delete_cache_entries(
            directory.cache_dir, directory.entries & fresh.entries
        )
        _rewrite_manifests(fresh)
    return freed
...
def _plan_directory(cache_dir: Path) -> DirectoryPrunePlan:
    ...
    for key in keys:
        try:
            ...
            # Bytes that read as no manifest, or that cannot be read at all,
            # list nothing, so deleting the entries they cover rests on
            # another manifest's word alone.
        ...
        if live_nodes is None:
            skips.append(
                f"{notebook.name} does not read as a whole notebook, so its "
                "entries are kept."
            )
            live |= manifest.entries()
            continue
...
def _live_nodes(notebook: Path) -> set[str] | None:
    ...
    notebook could not be read whole: it did not compile, or it parsed with
    errors that can drop a cell. What it holds is then unknown, never
    empty.
    """
    ...
    if status.status == "empty":
        # A file emptied to nothing, or to comments and a docstring, holds
        # no code that can produce an entry.
        return set()
    if (
        status.status not in ("valid", "has_warnings")
        or status.notebook is None
    ):
        # A parse with errors can leave out a cell that Python would still
        # run, and a missing cell reads as dead code.
        return None
...
def _unattested_siblings(cache_dir: Path, keys: Collection[str]) -> list[str]:
    ...
    for path in sorted(children):
        if (
            not path.is_file()
            or not MarimoPath.is_valid_path(path)
            or manifest_name(path) in keys
        ):
            continue
        if not os.access(path, os.R_OK):
            # What cannot be read may well be a notebook writing here.
            unattested.append(
                f"{path.name} shares this cache and cannot be read."
            )
        elif is_marimo_notebook(path):
            unattested.append(
