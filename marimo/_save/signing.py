...
# `verify` was the value the merged signing core (#10123) accepted on the
# `LazyLoader` kwarg, before this config surface named the setting
# `cache.verification`. Kept as an alias so that kwarg keeps working.
VERIFICATION_ALIASES = {"verify": "on"}
...
def normalize_verification(raw: Any, *, strict: bool = False) -> str:
    """Canonicalize a verification posture.

    Resolves the `verify` alias. With `strict=False` (config files) an
    unrecognized value warns and degrades to the default rather than breaking
    the session; with `strict=True` (a caller-supplied kwarg) it raises. The
    default is fail-safe — unverifiable entries recompute — so the degrade path
    never widens what gets deserialized.
    """
    if isinstance(raw, str):
        canonical = VERIFICATION_ALIASES.get(raw, raw)
        if canonical in VALID_VERIFICATIONS:
            return canonical
...
def normalize_fingerprint(fp: str) -> str:
    ...
    # canonical form emitted by fingerprint() — a permanent silent miss.
