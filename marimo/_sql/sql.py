    ...
    *,
    ...
    try:
        ...
        if is_sql_parse_error(e):
            # Get enhanced error with EXPLAIN
            error_message = None
            dialect = sql_engine.dialect.lower().strip()
            if dialect == "duckdb":
                _, error_message = sql_engine.execute_in_explain_mode(
                    query.strip()
                )
            ...
            # Raise custom exception to be captured by the runtime
