...
def test_cli_edit_disable_file_downloads(temp_marimo_file: str) -> None:
    """Test that --disable-file-downloads flag sets the config correctly."""
    port = _get_port()
    p = subprocess.Popen(
        [
            "marimo",
    ...
    contents = _try_fetch(port)
    _check_contents(p, b'"mode": "edit"', contents)
    _check_contents(p, b'"disable_file_downloads": true', contents)


def test_cli_run_disable_file_downloads(temp_marimo_file: str) -> None:
    """Test that --disable-file-downloads flag works with run command."""
    port = _get_port()
    p = subprocess.Popen(
        [
            "marimo",
