...
def test_lazy_docstring() -> None:
    """Test the MarkdownDocstring class."""

    # Test with regular docstring
    regular_doc = "Regular docstring"
    lazy = MarkdownDocstring(regular_doc)
    assert str(lazy) == "Regular docstring"
    assert repr(lazy) == "Regular docstring"
    ...
    """
    lazy_google = MarkdownDocstring(google_doc)
    converted = str(lazy_google)

    # The conversion is done by google_docstring_to_markdown
    # We should at least verify it's different from the input
    assert converted != google_doc
    assert "Function description." in converted

    # Test with None
    lazy_none = MarkdownDocstring(None)
    assert str(lazy_none) == ""

    lazy = MarkdownDocstring("Args: something")

    # First call should use conversion
    assert str(lazy) == "Summary\n-----------\n\nArgs: something"
