...
class TestPyArrowRecordBatchManagerFactory(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.factory = PyArrowTableManagerFactory()
        self.data = pa.RecordBatch.from_pandas(
            pd.DataFrame(
                {
                    "A": [1, 2, 3],
                    "B": ["aaa", "b", "c"],
    ...
    def test_get_field_types(self) -> None:
        ...
        complex_data = pa.RecordBatch.from_pandas(
            pd.DataFrame(
                {
                    "A": [1, 2, 3],
                    "B": ["aaa", "b", "c"],
    ...
    def test_apply_formatting(self) -> None:
        ...
        formatted_data = self.manager.apply_formatting(format_mapping)
        expected_data = pa.RecordBatch.from_pandas(
            pd.DataFrame(
                {
                    "A": [2, 4, 6],
                    "B": ["AAA", "B", "C"],
    ...
    def test_apply_formatting_with_empty_table(self) -> None:
        ...
        empty_data = pa.RecordBatch.from_pandas(pd.DataFrame({}))
        manager = self.factory.create()(empty_data)
