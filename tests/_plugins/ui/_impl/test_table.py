...
class TestCalculateTopKRowsCaching:
    def test_calculate_top_k_rows_caching(self) -> None:
        ...
        # Create a counter to track actual calculations
        calculation_count = 0

        def mock_calculate_top_k_rows(
            column: str, k: int
        ) -> list[tuple[Any, int]]:
            del k
        ...
        table._searched_manager.calculate_top_k_rows = (
            mock_calculate_top_k_rows  # type: ignore
        )
        table._manager.calculate_top_k_rows = mock_calculate_top_k_rows  # type: ignore

        # First call should compute the result
        result1 = table._calculate_top_k_rows(
            CalculateTopKRowsArgs(column="age", k=2)
        )
        assert calculation_count == 1  # First calculation

        # Second call with same args should use cache
        result2 = table._calculate_top_k_rows(
            CalculateTopKRowsArgs(column="age", k=2)
        )
        assert calculation_count == 1  # No new calculation
        assert result1 is result2  # Same object due to caching

        # Different k value should compute new result
        result3 = table._calculate_top_k_rows(
            CalculateTopKRowsArgs(column="age", k=3)
        )
        assert calculation_count == 2  # New calculation
        assert result3 is not result1

        # Different column should compute new result
        result4 = table._calculate_top_k_rows(
            CalculateTopKRowsArgs(column="name", k=2)
        )
        assert calculation_count == 3  # New calculation
        assert result4 is not result1

        # After search, cache should be invalidated because searched_manager changes
        table._search(
            SearchTableArgs(
                page_size=10,
                page_number=0,
        ...
        # Need to mock the new searched_manager
        table._searched_manager.calculate_top_k_rows = (
            mock_calculate_top_k_rows  # type: ignore
        )
        result5 = table._calculate_top_k_rows(
            CalculateTopKRowsArgs(column="age", k=2)
        )
        assert calculation_count == 4  # New calculation after search
        assert (
            result5 is not result1
        )  # Should be new result due to manager change
