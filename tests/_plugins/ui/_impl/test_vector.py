...
def test_vector_numpy_value_returns_ndarray():
    """When constructed from np.ndarray, .value returns np.ndarray."""
    import numpy as np

    v = ui.vector(np.array([1, 2, 3]))
    assert isinstance(v.value, np.ndarray)
    np.testing.assert_array_equal(v.value, np.array([1, 2, 3]))


def test_vector_numpy_convert_value_returns_ndarray():
    """_convert_value returns ndarray when constructed from ndarray."""
    import numpy as np

    v = ui.vector(np.array([1, 2, 3]))
    result = v._convert_value([[5], [6], [7]])
    assert isinstance(result, np.ndarray)
    np.testing.assert_array_equal(result, np.array([5, 6, 7]))


def test_vector_list_value_returns_list():
...
def test_vector_numpy_update_returns_ndarray():
    ...
    v._update([[10], [20], [30]])
    assert isinstance(v.value, np.ndarray)
    np.testing.assert_array_equal(v.value, np.array([10, 20, 30]))


def test_vector_numpy_transposed_returns_ndarray():
    """Transposed numpy vector still returns ndarray."""
    import numpy as np

    v = ui.vector(np.array([1, 2, 3]), transpose=True)
    assert isinstance(v.value, np.ndarray)
    np.testing.assert_array_equal(v.value, np.array([1, 2, 3]))
