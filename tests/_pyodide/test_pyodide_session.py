...
async def test_find_packages_from_island_notebook(
    ...
    # Generated by the frontend from a Python-rendered island payload.
    code = (
        Path(__file__).parent.parent
        / "_islands/snapshots/notebook-dependencies.txt"
    ).read_text(encoding="utf-8")

    assert pyodide_session.find_packages(code) == ["cowsay", "rich[jupyter]"]
    mock_pyodide.code.find_imports.assert_not_called()
