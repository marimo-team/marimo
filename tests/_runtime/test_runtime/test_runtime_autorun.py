...
class TestAutorunKernel:
    # Test Basic Reactivity
    async def test_triangle(self, k: Kernel) -> None:
        # x
        # x --> y
        # x, y --> z
        await k.run(
    ...
    async def test_set_ui_element_value(self, k: Kernel) -> None:
    ...
    async def test_set_local_var_ui_element_value(self, k: Kernel) -> None:
    ...
    async def test_creation_with_ui_element_value(self, k: Kernel) -> None:
    ...
    async def test_clear_multiple_definition_error(self, k: Kernel) -> None:
        await k.run(
            [
                ExecutionRequest(cell_id="0", code="x=0"),
                ExecutionRequest(cell_id="1", code="x=1"),
    ...
    async def test_new_errors_update_old_ones(self, k: Kernel) -> None:
        await k.run([ExecutionRequest(cell_id="0", code="x=0")])
    ...
    async def test_cycle_error(self, k: Kernel) -> None:
        await k.run(
            [
    ...
    async def test_break_cycle_error_with_delete(self, k: Kernel) -> None:
        await k.run(
            [
                ExecutionRequest(cell_id="0", code="x=y"),
                ExecutionRequest(cell_id="1", code="y=x"),
    ...
    async def test_delete_nonlocal_error(self, k: Kernel) -> None:
        await k.run(
            [
                ExecutionRequest(cell_id="0", code="x=0"),
    ...
    async def test_syntax_error(self, k: Kernel) -> None:
        await k.run(
            [
                ExecutionRequest(cell_id="0", code="x=0"),
...
class TestDisableAutorunKernel:
