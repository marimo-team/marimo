...
def test_create_and_register_with_expiration(
    run_mode_kernel: MockedKernel,  # noqa: ARG001
) -> None:
    import time
    ...
    vfile = VirtualFile.create_and_register(
        b"ephemeral data", "bin", expires_after_seconds=0.1
    )

    assert vfile.filename in ctx.virtual_file_registry.registry
    assert read_virtual_file(vfile.filename, 14) == b"ephemeral data"

    # Wait for the timer to fire
    time.sleep(0.3)

    assert vfile.filename not in ctx.virtual_file_registry.registry


def test_create_and_register_without_expiration_does_not_expire(
    run_mode_kernel: MockedKernel,  # noqa: ARG001
) -> None:
    import time
    ...
    vfile = VirtualFile.create_and_register(b"persistent data", "txt")

    assert vfile.filename in ctx.virtual_file_registry.registry

    time.sleep(0.2)

    # Should still be in the registry — no expiration set
    assert vfile.filename in ctx.virtual_file_registry.registry
    assert read_virtual_file(vfile.filename, 15) == b"persistent data"


def test_registry_add_with_expiration(
    run_mode_kernel: MockedKernel,  # noqa: ARG001
) -> None:
    import time

    ctx = get_context()
    registry = ctx.virtual_file_registry

    vfile = VirtualFile(filename="test-expire.bin", buffer=b"hello")
    registry.add(vfile, ctx, expires_after_seconds=0.1)

    assert registry.has("test-expire.bin")
    assert registry.storage.has("test-expire.bin")

    time.sleep(0.3)

    assert not registry.has("test-expire.bin")
    assert not registry.storage.has("test-expire.bin")
