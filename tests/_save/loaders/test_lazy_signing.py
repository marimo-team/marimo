...
class TestVerificationAndCapabilityValidation(_FileStoreLoaderTest):
    ...
    def test_legacy_verify_value_is_accepted_as_on(self) -> None:
        """`verify` was the shipped value before the setting was renamed."""
        loader = LazyLoader("test", store=self.store, verification="verify")
        assert loader.verification == "on"

    def test_legacy_verify_value_accepted_via_setattr(self) -> None:
        loader = LazyLoader("test", store=self.store, verification="off")
        loader.verification = "verify"
        assert loader.verification == "on"
