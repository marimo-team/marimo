...
def test_open_returns_absolute_one_time_uri(
    ...
    remote = TestClient(
        app, client=("192.0.2.10", 50000), follow_redirects=False
    ).get(target, headers={"X-Forwarded-For": "127.0.0.1"})
    assert remote.status_code == 303
    assert "/auth/login" in remote.headers["location"]
    assert "set-cookie" not in remote.headers
...
def test_execute_reuses_scratchpad_sse_after_resume(
    client: TestClient,
) -> None:
    ...
    assert re.fullmatch(r"sess-[0-9a-hjkmnp-tv-z]{16}", session.stable_id)
    execution_url = (
        f"/api/marimo/v1/sessions/{discovered_session['session_id']}/execute"
    )
    with patch.object(
        session, "connection_state", return_value=ConnectionState.ORPHANED
    ):
        assert (
            session_manager._resume_strategy.try_resume(
                SessionId("resumed-browser-session"),
                session.initialization_id,
            )
            is session
        )
    assert SessionId("discovery-session") not in session_manager.sessions
    assert (
        session_manager.get_session(SessionId("resumed-browser-session"))
        is session
    )
    assert (
