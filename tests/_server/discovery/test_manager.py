...
async def test_catalog_reports_incomplete_scan(
    tmp_path: Path, monkeypatch: MonkeyPatch, max_depth: int
) -> None:
    monkeypatch.setattr(
        "marimo._server.files.directory_scanner.DirectoryScanner.MAX_DEPTH",
        max_depth,
    )
    nested = tmp_path / "nested"
    nested.mkdir()
    (nested / "notebook.py").write_text(MARIMO_APP)
    session_manager = get_mock_session_manager()
    session_manager.workspace = DirectoryWorkspace(
        str(tmp_path), include_markdown=False
    )
    ...
    (project,) = (await manager.catalog()).projects
    assert project.truncated is (max_depth == 0)
    assert len(project.notebooks) == max_depth
