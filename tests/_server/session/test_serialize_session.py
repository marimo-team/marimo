...
def test_serialize_basic_session(session_view: SessionView):
    ...
    view = session_view
    view.cell_operations[CellId_t("cell1")] = CellOpNotification(
...
def test_serialize_notebook_multiple_cells(session_view: SessionView):
    ...
    view.cell_operations[cell_id1] = CellOpNotification(
        cell_id=cell_id1,
        status="idle",
        output=None,
        console=[],
    ...
    view.cell_operations[cell_id2] = CellOpNotification(
        cell_id=cell_id2,
        status="idle",
        output=CellOutput(
            channel=CellChannel.OUTPUT,
...
class TestSessionCacheManager:
    ...
    async def test_read_session_view_cache_hit(
        ...
        view.cell_operations["cell2"] = CellOpNotification(
            cell_id="cell2",
            status="idle",
            output=CellOutput(
                channel=CellChannel.OUTPUT,
