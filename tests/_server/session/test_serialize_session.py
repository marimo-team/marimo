...
def test_serialize_basic_session(session_view: SessionView):
    ...
    view = session_view
    view.cell_operations[CellId_t("cell1")] = CellNotification(
...
def test_serialize_session_with_console(session_view: SessionView):
    ...
    view = session_view
    view.cell_operations["cell1"] = CellNotification(
        cell_id="cell1",
        status="idle",
        output=None,
        console=[
...
def test_serialize_notebook_basic(session_view: SessionView):
    ...
    # Add to session view
    view.cell_operations[cell_id] = CellNotification(
        cell_id=cell_id,
        status="idle",
        output=CellOutput(
            channel=CellChannel.OUTPUT,
...
def test_serialize_notebook_multiple_cells(session_view: SessionView):
    ...
    view.cell_operations[cell_id1] = CellNotification(
        cell_id=cell_id1,
        status="idle",
        output=None,
        console=[],
    ...
    view.cell_operations[cell_id2] = CellNotification(
        cell_id=cell_id2,
        status="idle",
        output=CellOutput(
            channel=CellChannel.OUTPUT,
...
class TestSessionCacheManager:
    ...
    async def test_read_session_view_cache_hit(
        ...
        view.cell_operations["cell2"] = CellNotification(
            cell_id="cell2",
            status="idle",
            output=CellOutput(
                channel=CellChannel.OUTPUT,
