...
def test_add_storage_namespaces(session_view: SessionView) -> None:
    ...
    ns_by_name = {
        ns.name: ns for ns in session_view.storage_namespaces.namespaces
    }
    # Updated
    assert (
    ...
    assert session_view.storage_namespaces in session_view.notifications
...
def test_storage_namespaces_filtered_by_variables(
    ...
    assert len(session_view.storage_namespaces.namespaces) == 2
